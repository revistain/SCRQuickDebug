...
typedef struct TreeNode {
    ...
    TreeNode(uint32_t _func_number, uint32_t _func_time, uint32_t _depth, uint32_t _padding)
        : func_number(_func_number), func_time(_func_time), call_count(0), depth(_depth), padding(_padding), parent(nullptr) {}
    ...
    TreeNode* addChild(TreeNode* _parent, uint32_t _func_number, uint32_t _func_time, uint32_t _depth, uint32_t _padding) {
        ...
        TreeNode child = TreeNode(_func_number, _func_time, _depth, _padding);
...
public:
    ...
    Tree(): root(TreeNode(0xEDACEDAC, 0, 0, 0)){
    }
    ...
    void printTree(std::unique_ptr<Variables>& var_ptr, TreeNode* node, uint32_t depth=0) const {
        ...
        if (node->func_number != 0xEDACEDAC) {
            if (node->call_count != 0) {
                std::cout << std::dec << var_ptr->strtable.str[node->func_number] << " / " << (uint32_t)(node->func_time / node->call_count) << std::endl;
            }
            else {
